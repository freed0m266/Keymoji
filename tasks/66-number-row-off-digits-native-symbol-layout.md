...
- Refaktor: vyčlenit `makeDigitKeys() -> [Key]` (klávesy s `.standard` weight, ID `number.<digit>`, alternativy z `numberRowMapping`). `makeNumberRow()` ho zabalí do řádku s ID `"numberRow"`; symbol primary native ho zabalí do řádku se standardním ID. Klávesy mají stejné ID v obou, což je OK — number row a symbol page se nikdy nezobrazí současně (number row je v native configu z definice vypnutý).
